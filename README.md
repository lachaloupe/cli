...
go get github.com/lachaloupe/cli@v0.1.5

# tool-only dependency used by go generate
go get -tool github.com/lachaloupe/cli/cmd/cligen@v0.1.5
```
...
That means a subcommand can reach back to any parent's args.

When a command has both an args struct and subcommands, `cligen` generates a typed helper:

```go
// Generated by cligen for the root DockerArgs.
func DockerArgsFrom(ctx context.Context) DockerArgs {
	return ctx.Value(cli.Args("/")).(DockerArgs)
}
```

Subcommand handlers use it directly:

```go
func RunImageLs(ctx context.Context, args image.LsArgs) error {
	root := DockerArgsFrom(ctx)
	if root.Debug {
		// ...
	}
	return nil
